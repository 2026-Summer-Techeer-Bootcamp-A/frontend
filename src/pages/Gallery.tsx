...
const frontendScreens: Item[] = [
  // 앱 탭
  { to: '/', path: '/', title: '홈 · 대시보드', sub: '점수화 요약 · What-if · 주간 펄스 (모바일) / 멀티컬럼 대시보드 (PC)', swatch: 'linear-gradient(135deg,#21447c,#2f61b8)' },
  { to: '/market', path: '/market', title: '채용 시장', sub: '기회 사분면 · 수요 · co-occurrence · 추이 · 산업', swatch: 'linear-gradient(135deg,#2f61b8,#4a7bd0)' },
  { to: '/map', path: '/map', title: '지도', sub: 'Leaflet 국내 마커 · 클러스터 · 위치 권한 시트', swatch: 'linear-gradient(135deg,#2f61b8,#218a58)' },
  { to: '/resume', path: '/resume', title: '마이', sub: '이력서 · 내 기술 · 로그인 상태 · 설정 진입', swatch: 'linear-gradient(135deg,#2f61b8,#8fb0e2)' },
  ...
  { to: '/tech/TypeScript', path: '/tech/:name', title: '기술 세부', sub: '점유율 · 연차별 · 동반기술 · 과거vs현재', swatch: 'linear-gradient(135deg,#2f61b8,#7aa0dc)' },
  ...
  // 인증
  { to: '/splash', path: '/splash', title: '스플래시', sub: '로고 인트로 · 로그인 여부 따라 자동 진입', swatch: 'linear-gradient(135deg,#17306a,#2f61b8)' },
  { to: '/login', path: '/login', title: '로그인', sub: 'PC 풀스크린 분할 · 소셜(준비 중)', swatch: 'linear-gradient(135deg,#2f61b8,#4a7bd0)' },
  { to: '/signup', path: '/signup', title: '회원가입', sub: '이메일 · 비밀번호 · 닉네임 · 유효성', swatch: 'linear-gradient(135deg,#2f61b8,#5a86cf)' },
  // 설정
  { to: '/settings', path: '/settings', title: '설정 홈', sub: 'iOS 그룹 리스트 · 계정/정보/로그아웃', swatch: 'linear-gradient(135deg,#21447c,#8fb0e2)' },
  { to: '/settings/account', path: '/settings/account', title: '계정 관리', sub: '닉네임 수정 · 비번변경/탈퇴(준비 중)', swatch: 'linear-gradient(135deg,#2f61b8,#7aa0dc)' },
  { to: '/settings/notifications', path: '/settings/notifications', title: '알림 설정', sub: 'iOS 토글 4종 · 즉시 저장', swatch: 'linear-gradient(135deg,#2f61b8,#218a58)' },
...
const widgetDesigns: Item[] = [
  { to: '/signal', title: '당신의 시장 좌표', sub: '실데이터 기술 지형 · 수요×성장', swatch: 'linear-gradient(135deg,#12141a,#2f61b8)' },
  ...
  { to: '/signal/feed', title: '내게 맞는 공고 타임라인', sub: '실 publishedAt · 스트리밍 피드', swatch: 'linear-gradient(135deg,#12141a,#2f61b8)' },
