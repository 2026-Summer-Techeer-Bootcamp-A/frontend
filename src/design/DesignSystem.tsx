...
import {
  ...
  Navigation, Send, ThumbsUp, ThumbsDown,
  TrendingUp, Target,
} from 'lucide-react'
...
/** 어시스턴트 스레드 — 조회 중(은은한 글로우) → 문장 단위 스트리밍 → 문장마다 바로 근거 칩.
 * "AI가 답을 지어낸다"가 아니라 "우리 데이터를 문장 단위로 조회해 답한다"는 걸 구조로 드러냄. */
const AX_SEGMENTS: { text: string; cite: string }[] = [
  { text: '최근 30일 React 공고는 482건으로 8% 늘었어요.', cite: '공고 482건 · React · 2026-07-07' },
  { text: '보유 기술 기준 커버리지는 73%예요.', cite: '상위 20개 기술 중 보유 비율(K=20) · SQL 집계' },
]
function AssistantThreadDemo() {
  const [phase, setPhase] = useState<'retrieving' | 'streaming' | 'done'>('retrieving')
  ...
  useEffect(() => {
    ...
    const seg = AX_SEGMENTS[segIndex]
    if (!seg) { setPhase('done'); return }
    const chars = Array.from(seg.text)
    if (charIdx < chars.length) {
      const t = window.setTimeout(() => setCharIdx((n) => n + 1), 32)
      return () => window.clearTimeout(t)
    }
    ...
    const t = window.setTimeout(() => { setSegIndex((n) => n + 1); setCharIdx(0) }, 300)
    return () => window.clearTimeout(t)
  }, [phase, segIndex, charIdx, citeShown])

  const generating = phase !== 'done'

  return (
    <div className={`ds-card ax-card${generating ? ' generating' : ''}`}>
      <div className="demo-label">어시스턴트 헤더 + 채팅 스레드 <span style={{ fontWeight: 500, color: 'var(--muted)' }}>· Apple Intelligence풍 글로우 · 문장마다 근거</span></div>
      <div className="ax-card__halo" aria-hidden="true" />
      <div className="ax-head" style={{ marginBottom: 14 }}>
        <div className="ax-avatar ax-avatar--orb"><span className="ax-avatar__orb" /></div>
        <div>
          <div className="nm">커리어 어시스턴트</div>
          <div className="st">{phase === 'retrieving' ? '공고 데이터 조회 중…' : '데이터 기반 응답 · 공고 12,480건'}</div>
        </div>
      </div>
            ...
            {AX_SEGMENTS.map((seg, i) => {
              ...
              return (
                <span className="ax-seg" key={i}>
                  {shownChars.map((c, ci) => (
                    <span className="ax-char" key={ci}>{c}</span>
                  ))}
      ...
      <button className="btn btn--primary btn--sm" style={{ marginTop: 12 }} onClick={() => setK((x) => x + 1)}>다시 재생</button>
    </div>
  )
}
