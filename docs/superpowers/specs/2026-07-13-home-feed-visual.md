...
카드)에 얹히는 **차분하지만 밀도 높은 정보 피드**. 색은 세 곳에만 쓴다.
(1) 공고 **매치율 링**, (2) **D-day 긴급도(D-3 이하)**, (3) 공고 카드의 **보유 스킬 칩**.
나머지는 전부 잉크/뮤트/라인의 무채색. 타임라인 느낌은 카드 장식이 아니라
**날짜 구분선**으로 만든다. 이모지는 코드/카피 어디에도 쓰지 않는다.
...
홈 피드는 셸 콘텐츠 영역(`.dshell__content`, 흰 라운드 카드, padding 18px, 자체 스크롤)
...
정적 카드는 hover 변화를 주지 않는다(프로필/이력서/뉴스/미니). **클릭 가능한 공고 카드와
미니 요약 카드**만 hover/active 반응을 준다.
...
- 완성도 행 `__meter` (margin-top 12px):
  ...
  - 바: `height:8px; border-radius:5px; background:#eceef3;` 채움 `i { background:var(--c-accent); }`
    (그라디언트 대신 단색 near-black. 8px, 라운드 5px)
...
- 로우 `__row`: `display:flex; align-items:center; gap:12px; padding:12px 12px;
  border-radius:12px;` hover `background:#f7f7f8;`
...
한 화면에서 칩 의미를 색이 아니라 **채움 방식**으로 구분한다.

| 클래스 | 배경 | 텍스트 | 보더 | 용도 |
|---|---|---|---|---|
| `.hfeed-chip` (소프트) | `var(--accent-50)` #f0f0f1 | `var(--accent-700)` #18181b | `var(--accent-100)` #e4e4e7 | 이력서 요약의 보유 스킬 나열 |
| `.hfeed-chip--owned` (필드) | `var(--c-accent)` #0b0b0c | `#fff` | transparent | 공고 카드: **내가 보유한** 요구 스킬 |
...
- 채움: `background:conic-gradient(var(--c-accent) 0 <rate>%, #eceef3 <rate>%);`
...
- 리스트 `__list`: margin-top 8px. 각 아이템 `__item`은
  ...
  border-bottom:1px solid var(--line2, #f0f0f1);` hover `background:#f7f7f8;`
  ...
  - **HN `__item--hn`**:
    ...
    - 메타 `__meta`: `margin-top:6px; font:500 11.5px; color:var(--c-muted);
      font-variant-numeric:tabular-nums;` 형식 "포인트 342 · 댓글 128".
  ...
  - **GitHub Trending `__item--gh`**:
    ...
    - 메타 `__meta`: `margin-top:6px; font:600 11.5px; color:var(--c-muted);
      font-variant-numeric:tabular-nums;` 형식 "언어점 TypeScript · 스타 1.2k".
      언어점 `__langdot`: `width:8px; height:8px; border-radius:50%; background:#a1a1aa;`
      (모노크롬 — 언어별 색 대신 뉴트럴 점 + 텍스트).
...
- 숫자 `__num`: `24px/800; color:var(--c-ink); font-variant-numeric:tabular-nums;`
...
- **리스트 변형 `.hfeed-mini--list`** (급상승 스킬 Top3):
  - 라벨 아래 로우 `__row` ×3: `display:flex; align-items:center; gap:8px; padding:6px 0;
    ...
    - 증가율 `__delta`: `11.5px/800; color:#1f9d57;` 예 "+38%"  (상승 = 모노 그린 톤)
- 카드 목록:
...
| 뉴스 리스트 로우 | `background:#f7f7f8` | — | outline 동일 |
| 북마크 토글 | `background:accent-50` | `scale(.95)` | outline 동일 |
| 빠른 링크 로우 | `background:#f7f7f8` | — | outline 동일 |
...
- 피드 빈 상태 `.hfeed-empty` (필터 결과 0건):
  `text-align:center; padding:40px 20px; background:#f6f8fc; border-radius:20px;
  color:var(--c-muted);`
...
4. 공고 카드: 헤드(로고+회사+메타+제목+링) / 시간·D-day 행 / 칩 행(카테고리+보유필드+미보유아웃라인) / 푸터(매치요약+상세+북마크). 비로그인 degrade 분기.
5. D-day: `dday <= 3`이면 `.is-urgent`(코랄), 마감 없으면 `상시채용`.
6. 링: conic-gradient에 정수 rate 주입, 홀+숫자, aria-label.
7. 우측: 뉴스 패널(3탭 세그먼트 + 소스별 아이템 + 갱신 캡션) → 미니카드 2~3장(리스트 변형 포함).
8. 상태: 스켈레톤(카드/뉴스/프로필), 빈/에러(피드·뉴스), 뉴스 stale 캡션.
9. hover/active/focus-visible 전부 부여. `prefers-reduced-motion` 대응.
10. 색 사용 감사: accent 채도(near-black 필드/코랄/앰버)는 매치 링·D-day 긴급·보유 칩·CTA 주버튼에만. 나머지 무채색.
